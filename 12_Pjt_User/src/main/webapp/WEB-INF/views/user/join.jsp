...
<script>
	...
	function fnCheckId() {
		...
		$('#id').on('keyup', function() {
			...
			// 정규식 (5~40자, 소문자+숫자+하이픈+언더바 사용 가능, 첫 글자는 소문자+숫자 사용)
			let regId = /^[a-z0-9][a-z0-9-_]{4,39}$/;
			
			// 정규식 검사
			verifyId = regId.test(id);
			if(verifyId == false){
				$('#msgId').text('5~40자, 소문자+숫자+하이픈+언더바 사용 가능, 첫 글자는 소문자+숫자 사용 가능');
				return; // 함수 종료 (이후에 나오는 ajax(중복체크) 실행을 막기 위해서)
			}
	...
	function fnCheckEmail(){
		...
		$('#btnGetCode').on('click', function() {
			...
			new Promise(function(resolve, reject){
				...
				if(verifyEmail == false) {
					reject(1); // catch에 정의된 function을 호출한다.
					return;
				}
				...
				$.ajax({
					...
					success: function(resData){ // resData = {"enableEmail": true} 또는 {"enableEmail":false}
						...
						if(resData.enableEmail) {
							resolve();	// then에 정의된 function을 호출한다.						
						}else {
							reject(2);	// catch에 정의된 function을 호출한다.
						}
					}
				...
				$.ajax({
					...
					data: 'email=' + email,
					dataType: 'json',
					success: function(resData) { // resData = {"authCode": "6T43G9"} 사용자에게 전송한 인증 코드를 의미
						...
						$('#btnVerifyCode').on('click', function() {
							...
							if(verifyEmail) { // 사용자에게 전송한 인증코드 == 입력한 인증코드
								alert('인증되었습니다.');
							}else{
								alert('인증에 실패했습니다.');
							}
		...
		new Promise(function(resolve, reject) {
			...
			// number == 1인 경우 정규식 실패 메세지
			// number == 2인 경우 이메일 중복 체크 실패 메세지
  ...
  <div>
    ...
    <form id="frmJoin" method="post" action="${contextPath}/user/join.do">
      ...
      <div>
        ...
        <select name="birthyear" id="birthyear">100년 전 ~ 내년</select>
        <select name="birthmonth" id="birthmonth">1~12월</select>
        <select name="birthdate" id="birthdate">1~31일(월별로)</select>       
      </div>
      
      <div>
      	<input  type="text" onclick="execDaumPostcode()" name="postcode" id="postcode" placeholder="우편번호" readonly>

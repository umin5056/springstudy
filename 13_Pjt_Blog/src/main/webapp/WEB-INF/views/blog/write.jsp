...
<script>
	...
	function fnList() {
		location.href = getContextPath() + '/board/list.do';
	}
	...
	$(function(){
		$('#content').summernote({
			...
			callbacks: {
				onImageUpload: function(files) { 			// summernote 편집기에 이미지를 올리면 해당 이미지의 정보는 funcion의 매개변수 files로 전달된다.
					for(let i=0; i<files.length; i++) { // files 배열을 순회하면서 이미지를 하나씩 처리한다.
						...
						$.ajax({													// 가상 form에 저장도니 이미지를 HDD에 저장하고 저장된 경로(매핑값)와 이름을 받아오는 ajax
							...
							data: formData,
							dataType: 'json',
							success: function(resData) {		// resData = {"src": "/app13/imageLoad/abcdef.jpg"}
								$('#content').summernote('insertImage', resData.src);	// content에 <img src="/app13/imageLoad/abcdef.jpg">태그가 추가된다.
																																			// content에 추가된 img태그의 src속성 값은 servlet-context.xml의 resources 태그에 의해서 /storage/summernote/abcdefg.jpg로 처리된다.	
